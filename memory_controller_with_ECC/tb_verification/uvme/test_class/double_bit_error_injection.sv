class double_bit_error_injection_test extends zmc_base_test;

  // Factory registration
  `uvm_component_utils(double_bit_error_injection_test)
 ...
 //local_intf l_f;
  ...
  // Constructor
  function new(string name = "double_bit_error_injection_test", uvm_component parent = null);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    `uvm_info(get_full_name(), "Inside the double_bit_error_injection_test test", UVM_MEDIUM)
    // Set the block handle
  seq.regmodel = env.regmodel; // block handle path

   wr_seq_inst.scenario=13;
   seq.scenario = 13;
...
//try -1
   ...
   join
    ...
    `uvm_info(get_full_name(), "Inside the double_bit_error_injection_test test done ", UVM_MEDIUM)
  endtask
