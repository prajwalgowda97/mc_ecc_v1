class random_rd_test extends zmc_base_test;

  // Factory registration
  `uvm_component_utils(random_rd_test)
   ecc_frontdoor_seq seq;
   int scenario;

  // Constructor
  function new(string name = "random_rd_test", uvm_component parent = null);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    `uvm_info(get_full_name(), "Inside the random_rd_test test", UVM_MEDIUM)
  ...
  seq.regmodel = env.regmodel; // block handle path
   ...
   join
    ...
    `uvm_info(get_full_name(), "Inside the random_rd_test test done ", UVM_MEDIUM)
  endtask
endclass
