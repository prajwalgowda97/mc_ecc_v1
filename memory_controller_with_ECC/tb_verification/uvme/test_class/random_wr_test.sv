class random_wr_test extends zmc_base_test;
  ...
  // Constructor
  function new(string name = "random_wr_test", uvm_component parent = null);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    `uvm_info(get_full_name(), "Inside the random_wr_test test", UVM_MEDIUM)
    // Set the block handle
  seq.regmodel = env.regmodel; // block handle path

   wr_seq_inst.scenario=15;
   seq.scenario = 15;
    // Start the sequence
   fork
   ...
   join
    ...
    `uvm_info(get_full_name(), "Inside the random_wr_test test done ", UVM_MEDIUM)
  endtask
endclass
