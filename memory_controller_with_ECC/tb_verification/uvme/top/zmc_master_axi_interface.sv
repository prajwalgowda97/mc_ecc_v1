


interface zmc_master_axi_interface( input logic zmc_top_clk,input logic zmc_top_rstn,input logic zmc_top_sw_rst);	                 

        logic	                      zmc_top_mem_init;
        logic[31:0]                     awaddr;
        logic[3:0]                      awlen;
        logic[1:0]                      awburst;
        logic                           awvalid;
        logic                           awready;

        logic[31:0]                       wdata;
        logic                             wlast;
        logic[3:0]                        wstrb;
        logic                             wvalid;
        logic                             wready;

        logic                             bvalid;
        logic                             bready;
        logic[1:0]                        bresp;

        logic[31:0]                       araddr;
        logic[3:0]                        arlen;
        logic[1:0]                        arburst;
        logic                             arvalid;
        logic                             arready;

        logic[31:0]                        rdata;
        logic                              rlast;
        logic[1:0]                         rresp;
        logic                              rvalid;
        logic                              rready;

        logic                              ECC_interrupt;
        logic[31:0]                        o_ECC_STAUS_REG;
        logic                              MEM_init_ACK;

              


//clocking block for master driver

   
   clocking master_driver_cb@(posedge zmc_top_clk);
        output zmc_top_rstn;
        output zmc_top_sw_rst;
        output zmc_top_mem_init;
        output awaddr, awlen, awburst, awvalid;
        input  awready;
        output wdata, wlast, wstrb, wvalid;
        input  wready;
        output bready;
        input  bvalid, bresp;
        output araddr, arlen, arburst, arvalid;
        input  arready;
        output rready;
        input  rdata, rlast, rresp, rvalid;
        input ECC_interrupt, o_ECC_STAUS_REG,MEM_init_ACK;

        
   endclocking

//clocking master_monitor_cb

    clocking master_monitor_cb@(posedge zmc_top_clk);

        input zmc_top_rstn;
        input zmc_top_sw_rst;
        input zmc_top_mem_init;
        input awaddr, awlen, awburst, awvalid, awready;
        input wdata, wlast, wstrb, wvalid, wready;
        input bready, bvalid, bresp;
        input araddr, arlen, arburst, arvalid, arready;
        input rready, rdata, rlast, rresp, rvalid;
        input ECC_interrupt, o_ECC_STAUS_REG,MEM_init_ACK;

    endclocking




endinterface
